...
test.describe('Mail Account Management Snackbar Notifications', () => {
  ...
  test('Shows snackbar on successful mail account creation', async ({ page }) => {
    ...
    await expect(snackbar.locator('.MuiAlert-message')).toContainText('Mail account "Test Mail Account" added');
  ...
  test('Shows snackbar on successful mail account update', async ({ page }) => {
    ...
    const editButton = page.locator('tbody tr').first().locator('button').filter({ has: page.locator('svg') }).nth(1);
    await editButton.click();

    await page.locator('.MuiDialog-root').getByLabel('Account Name').fill('Updated Name');
    await page.locator('.MuiDialog-root').getByRole('button', { name: 'Save' }).click();

    const snackbar = page.locator('.MuiSnackbar-root');
    await expect(snackbar).toBeVisible();
    await expect(snackbar.locator('.MuiAlert-message')).toContainText('Mail account updated successfully');
  });


  test('Shows snackbar on successful mail account deletion', async ({ page }) => {
    ...
    const deleteButton = page.locator('tbody tr').first().locator('button[color="error"], button').filter({ has: page.locator('svg[color="#f44336"]') }).or(
    ...
    const snackbar = page.locator('.MuiSnackbar-root');
    await expect(snackbar).toBeVisible();
    await expect(snackbar.locator('.MuiAlert-message')).toContainText('Mail account deleted successfully');
  });
  ...
  test('Refresh button updates connection status', async ({ page }) => {
    if (await page.locator('tbody tr').count() > 0) {
      const refreshButton = page.locator('tbody tr').first().locator('button[title*="Refresh"], button').first();
  ...
  test('Helper text shows for account name field', async ({ page }) => {
    await expect(page.locator('text=Friendly name to identify this mail account')).toBeVisible();
  });
  ...
  test('Table displays all required columns', async ({ page }) => {
    ...
    await expect(page.getByRole('columnheader', { name: 'IMAP Host', exact: true })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: 'IMAP Port', exact: true })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: 'Username', exact: true })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: 'SMTP Host', exact: true })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: 'SMTP Port', exact: true })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: 'IMAP Status', exact: true })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: 'SMTP Status', exact: true })).toBeVisible();
