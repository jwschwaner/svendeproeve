...
test.describe('Account Settings', () => {
  ...
  test.describe('Password Section', () => {
    ...
    test('should successfully change password', async ({ page, request }) => {
      ...
      const snackbar = page.locator('.MuiSnackbar-root');
      await expect(snackbar).toBeVisible();
      await expect(snackbar.locator('.MuiAlert-message')).toContainText('Password changed successfully');
    });
    ...
    test('should show error when current password is incorrect', async ({ page }) => {
      ...
      const snackbar = page.locator('.MuiSnackbar-root');
      await expect(snackbar).toBeVisible();
      await expect(snackbar.locator('.MuiAlert-message')).toContainText(/current password is incorrect/i);
    });
    ...
    test('should show error when password fields are empty', async ({ page }) => {
      await page.getByRole('button', { name: 'Change Password' }).click();
  ...
  test.describe('Delete Account Section', () => {
    ...
    test('should display delete account warning', async ({ page }) => {
      await expect(page.getByText('Deleting your account is permanent and cannot be undone')).toBeVisible();
      await expect(page.getByText('Delete all your personal data and profile')).toBeVisible();
    });

    test('should show error when confirmation text is incorrect', async ({ page }) => {
    ...
    test('should cancel deletion when clicking cancel', async ({ page }) => {
      ...
      await page.locator('.MuiDialog-root').getByRole('button', { name: 'Cancel' }).click();
  ...
  test.describe('Navigation', () => {
    test('should navigate between tabs', async ({ page }) => {
      ...
      await page.getByRole('button', { name: 'Delete Account' }).click();
      await expect(page.getByText('Deleting your account is permanent')).toBeVisible();
