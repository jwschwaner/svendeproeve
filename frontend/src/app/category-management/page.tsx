...
const DEFAULT_FORM: CategoryCreateData = {
  name: "",
  description: "",
  color: undefined,
};
...
export default function CategoryManagementPage() {
  ...
  const [selectedMailAccountIds, setSelectedMailAccountIds] = useState<
    string[]
  >([]);
  ...
  const [editSelectedMailAccountIds, setEditSelectedMailAccountIds] = useState<
    string[]
  >([]);
    ...
    try {
      await categoryApi.create(
        currentOrg.id,
        {
          ...
          mail_account_ids:
            selectedMailAccountIds.length > 0
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell
                  sx={{ color: "text.secondary", fontWeight: 600, width: 60 }}
                >
                  Color
                </TableCell>
                <TableCell sx={{ color: "text.secondary", fontWeight: 600 }}>
              ...
              {sortedCategories.length > 0 ? (
                sortedCategories.map((category) => (
                  ...
                  >
                    ...
                    <TableCell sx={{ color: "text.secondary" }}>
                      {category.mail_account_ids &&
                      category.mail_account_ids.length > 0
                        ? category.mail_account_ids
                            .map(
                              (id) =>
                                mailAccounts?.find((ma) => ma.id === id)
                                  ?.name || id,
                            )
                            .join(", ")
                        : "All"}
        ...
        <DialogContent>
          ...
          >
            <TextField
              ...
              fullWidth
              inputProps={{ "data-testid": "category-edit-name" }}
            />
            <TextField
              label="Description (for AI)"
              ...
              multiline
              rows={2}
              inputProps={{ "data-testid": "category-edit-description" }}
            />
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
              <Box
                sx={{
              ...
              {editForm.color && (
                <Button
                  size="small"
                  onClick={() =>
                    setEditForm((f) => ({ ...f, color: undefined }))
                  }
                  sx={{
                    textTransform: "none",
                    fontSize: "0.75rem",
                ...
                <FormGroup row>
                  {mailAccounts.map((ma) => (
                    <FormControlLabel
                      ...
                      control={
                        <Checkbox
                          checked={editSelectedMailAccountIds.includes(ma.id)}
                          onChange={() =>
                            toggleMailAccount(
                              ma.id,
                              editSelectedMailAccountIds,
                              setEditSelectedMailAccountIds,
                            )
                          }
                          disabled={isEditSubmitting}
                          size="small"
